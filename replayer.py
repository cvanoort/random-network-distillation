...
class EpisodeIterator(object):
    def __init__(self, filenames):
        if args["filter"] == "none":
            ...
            cond = lambda info: info["r"] < args["rew_max"] and (
                info["r"] > args["rew_min"]
            )
    ...
    def iterate(self):
        for filename in self.filenames:
            ...
            with open(filename, "rb") as f:
                yield from self.iterate_over_episodes_in_file(
                    ...
                    if "obs" in episode:
                        ...
                        if "rng_at_episode_start" in info:
                            ...
                            if hasattr(unwrapped_env, "scene"):
                                unwrapped_env.scene.np_random.set_state(
...
class Animation(object):
    ...
    def create_axes(self, episode):
        assert self.axes == {}
        keys = [
            key for key in episode.keys() if key not in ["acs", "infos", "obs", "info"]
        ]
        keys.insert(0, "obs")
    ...
    def run(self):
        ...
        if self.axes == {}:
            ...
            self.im = self.axes["obs"].imshow(
                self.process_frame(self.episode["obs"][0]), cmap="gray"
            )
            for key in self.axes:
                if key != "obs":
                    (line,) = self.axes[key].plot(self.episode[key], alpha=0.5)
                    dot = matplotlib.patches.Ellipse(
                        xy=(0, 0), width=1, height=0.0001, color="r"
                    )
            ...
            if self.j == 0:
                for key in self.axes:
                    if key != "obs":
                        ...
                        self.dots[key].height = (max_y - min_y) / 30.0
                        self.dots[key].width = n_timesteps / 30.0
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--display", type=str, default="game", choices=["game", "agent"]
    )
    parser.add_argument("--skip", type=int, default=0)
    parser.add_argument(
        "--room_number", type=lambda x: [int(_) for _ in x.split(",")], default=[15]
    )
    ...
    for machine_dir in all_machine_dirs:
        ...
        this_machine_other_folders = [
            f_
            for f_ in this_machine_other_folders
            if f_.startswith("openai-")
            and abs((date_from_folder(f_) - date_started).total_seconds()) < 3
        ]
        this_machine_other_folders = [
            os.path.join(machine_dir, f_) for f_ in this_machine_other_folders
        ]

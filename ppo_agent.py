...
class InteractionState(object):
    ...
    def __init__(self, ob_space, ac_space, nsteps, gamma, venvs, stochpol, comm):
        ...
        self.venvs = venvs
        assert all(
            venv.num_envs == self.lump_stride for venv in self.venvs[1:]
        ), "All venvs should have the same num_envs"
        ...
        self.buf_ob_last = {
            k: self.buf_obs[k][:, 0, ...].copy() for k in stochpol.ph_ob_keys
        }
...
class PpoAgent(object):
    ...
    def __init__(
        ...
        *,
        ...
        if comm_train is not None:
            ...
            self.comm_train, self.comm_train_size = (
                self.comm_log,
                self.comm_log.Get_size(),
            )
        ...
        with tf.variable_scope(scope):
            ...
            maxkl = 0.5 * tf.reduce_max(tf.square(neglogpac - self.ph_oldnlp))
            clipfrac = tf.reduce_mean(
                tf.to_float(tf.greater(tf.abs(ratio - 1.0), self.ph_cliprange))
            )
            ...
            logger.info(
                "PPO: using MpiAdamOptimizer connected to %i peers"
        ...
        self._losses = [
            ...
            approxkl,
            maxkl,
            self.stochpol.aux_loss,
            self.stochpol.feat_var,
            self.stochpol.max_feat,
            global_grad_norm,
        ]
        self.loss_names = [
            "tot",
    ...
    def update(self):
        ...
        for t in range(self.nsteps - 1, -1, -1):  # nsteps-2 ... 0
            if self.use_news:
                nextnew = (
                    self.I.buf_news[:, t + 1]
                    if t + 1 < self.nsteps
                    else self.I.buf_new_last
                )
            else:
                nextnew = 0.0  # No dones for intrinsic reward.
            nextvals = (
                self.I.buf_vpreds_int[:, t + 1]
                if t + 1 < self.nsteps
                else self.I.buf_vpred_int_last
            )
        ...
        for t in range(self.nsteps - 1, -1, -1):  # nsteps-2 ... 0
            ...
            nextvals = (
                self.I.buf_vpreds_ext[:, t + 1]
                if t + 1 < self.nsteps
                else self.I.buf_vpred_ext_last
            )
            nextnotnew = 1 - nextnew
            delta = (
                rews_ext[:, t]
        ...
        info = dict(
            ...
            ev_int=np.clip(
                explained_variance(self.I.buf_vpreds_int.ravel(), rets_int.ravel()),
                -1,
                None,
            ),
            ev_ext=np.clip(
                explained_variance(self.I.buf_vpreds_ext.ravel(), rets_ext.ravel()),
                -1,
                None,
            ),
        ...
        if verbose and self.is_log_leader:
            ...
            logger.info(
                ...
                % (
                    ...
                    samples,
                    samples // self.nminibatches,
                    samples * self.comm_train_size,
        ...
        while epoch < self.nepochs:
            ...
            ret = tf.get_default_session().run(
                self._losses + [self._train], feed_dict=fd
            )[:-1]
            if not self.testing:
    ...
    def env_get(self, l):
        ...
        if self.I.step_count == 0:
            ...
            out = self.I.env_results[l] = (
                ob,
                None,
                np.ones(self.I.lump_stride, bool),
                {},
            )
        else:
            ...
            for env_pos_in_lump, info in enumerate(infos):
                if "episode" in info:
                    ...
                    try:
                        ...
                        ipdb.set_trace()
                    self.I.buf_epinfos[env_pos_in_lump + l * self.I.lump_stride][
                        t
                    ] = info_with_places
        ...
        if t == self.nsteps - 1 and not self.disable_policy_update:
            ...
            for l in range(self.I.nlump):
                ...
                with logger.ProfileKV("policy_inference"):
                    (
                        _,
                        self.I.buf_vpred_int_last[sli],
                        self.I.buf_vpred_ext_last[sli],
                        _,
                        _,
                        _,
                    ) = self.stochpol.call(
            ...
            global_deque_mean = dict_gather(
                self.comm_log,
                {n: np.mean(dvs) for n, dvs in self.I.statlists.items()},
                op="mean",
            )
